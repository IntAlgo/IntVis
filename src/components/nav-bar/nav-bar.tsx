...
export const NavBar = ({ className }: NavBarProps) => {
    ...
    return (
        <div className={classNames(styles.root, className)}>
            <nav>
                <button onClick={()=>{setMode("add")}} className={styles["navbar-item"]}>add</button> |{' '}
                <button onClick={()=>setMode("edge")} className={styles["navbar-item"]} >edge//still in progress</button> | <button className={styles["navbar-item"]}>Contact Us</button> |  <button onClick={()=>{setMode("DFS")}} className={styles["navbar-item"]}>Visualize DFS</button>
